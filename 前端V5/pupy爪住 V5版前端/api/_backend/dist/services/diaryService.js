...
export class DiaryService {
    static async createDiary(userId, petId, data) {
        try {
    ...
    static async getUserDiaries(userId, page = 1, limit = 20) {
        try {
    ...
    static async getPublicDiaries(page = 1, limit = 20) {
        try {
            const offset = (page - 1) * limit;
            const [diariesRes, countRes] = await Promise.all([
                supabase
    ...
    static async updateDiary(diaryId, updates) {
        try {
            const { data, error } = await supabase
                .from('diaries')
                .update(updates)
    ...
    // 添加赞
    static async likeDiary(diaryId, userId) {
        try {
            // 检查是否已赞
    ...
    // 取消赞
    static async unlikeDiary(diaryId, userId) {
        try {
            await supabase
                .from('likes')
                .delete()
    ...
    // 添加评论
    static async addComment(diaryId, userId, content) {
        try {
    ...
    // 获取评论列表
    static async getComments(diaryId, page = 1, limit = 20) {
        try {
            const offset = (page - 1) * limit;
