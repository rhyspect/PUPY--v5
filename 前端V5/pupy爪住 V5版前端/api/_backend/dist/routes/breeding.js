...
router.post('/', authMiddleware, async (req, res) => {
    try {
        ...
        const result = await BreedingService.createBreedingRequest(req.user.user_id, sender_pet_id, receiver_id, receiver_pet_id, notes);
...
router.get('/', authMiddleware, async (req, res) => {
    try {
        ...
        const result = await BreedingService.getBreedingRequestsForUser(req.user.user_id, paginationParams.page, paginationParams.limit);
        res.json(result);
    }
    catch (error) {
...
router.patch('/:requestId/status', authMiddleware, async (req, res) => {
    try {
        ...
        const result = await BreedingService.updateBreedingRequestStatus(req.params.requestId, status);
        res.json(result);
    }
    catch (error) {
