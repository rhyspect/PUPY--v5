...
router.get('/feed', async (req, res) => {
    try {
        ...
        const result = await MarketService.getFeed(typeof category === 'string' ? category : undefined, typeof type === 'string' ? type : undefined, Number(limit));
        res.json(result);
    }
    catch (error) {
...
router.get('/category/:category', async (req, res) => {
    try {
        ...
        const result = await MarketService.getProductsByCategory(req.params.category, paginationParams.page, paginationParams.limit);
        res.json(result);
    }
    catch (error) {
...
router.get('/search', async (req, res) => {
    try {
        ...
        const paginationParams = getPaginationParams(req.query);
        const result = await MarketService.searchProducts(keyword, paginationParams.page, paginationParams.limit);
        res.json(result);
    }
    catch (error) {
...
router.get('/breeding/market', async (req, res) => {
    try {
        const paginationParams = getPaginationParams(req.query);
        const result = await MarketService.getBreedingMarket(paginationParams.page, paginationParams.limit);
        res.json(result);
    }
    catch (error) {
