...
export default function Messages({ onSelectChat, onViewOwner }: { onSelectChat: (owner?: any) => void, onViewOwner: (owner: any) => void }) {
            ...
            >
              {ownerMessages.map((msg) => (
                <div 
                  key={msg.id} 
                  onClick={() => {
                    const pet = PETS.find(p => p.owner.name === msg.name);
                    onSelectChat(pet?.owner);
                  }}
                ...
                {petSocialRecords.map((record) => (
                  <div key={record.id} className="bg-white rounded-[2.5rem] p-6 shadow-sm border border-slate-50 space-y-4">
                    ...
                    <div className="flex gap-3">
                      <button 
                        onClick={() => {
                          const pet = PETS.find(p => p.name === record.petName);
                          if (pet) onViewOwner(pet.owner);
                        }}
                        className="flex-1 py-3 bg-slate-50 hover:bg-slate-100 text-slate-500 text-xs font-bold rounded-2xl transition-colors flex items-center justify-center gap-2"
                      >
                      ...
                      <button 
                        ...
                        className="flex-1 py-3 bg-primary/10 hover:bg-primary/20 text-primary text-xs font-bold rounded-2xl transition-colors flex items-center justify-center gap-2"
                      >
