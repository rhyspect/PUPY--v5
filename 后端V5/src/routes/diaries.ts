...
// 获取公开日记
router.get('/public/feed', async (req: Express.Request, res: Express.Response) => {
  try {
    const paginationParams = getPaginationParams(req.query);
    const result = await DiaryService.getPublicDiaries(
      paginationParams.page,
      paginationParams.limit,
    );
...
// 获取单个日记
router.get('/:diaryId', async (req: Express.Request, res: Express.Response) => {
  try {
...
// 删除日记
router.delete('/:diaryId', authMiddleware, async (req: AuthRequest, res: Express.Response) => {
  try {
...
// 赞日记
router.post('/:diaryId/like', authMiddleware, async (req: AuthRequest, res: Express.Response) => {
  try {
    if (!req.user?.user_id) {
      return res.status(401).json({ success: false, error: '未认证', code: 401 });
    }
...
router.get(
  ...
  async (req: Express.Request, res: Express.Response) => {
    try {
      const paginationParams = getPaginationParams(req.query);
      const result = await DiaryService.getComments(
        req.params.diaryId,
        paginationParams.page,
        paginationParams.limit,
      );
